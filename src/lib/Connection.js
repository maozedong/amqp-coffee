const boundMethodCheck = function (instance, Constructor) { if (!(instance instanceof Constructor)) { throw new Error('Bound instance method accessed before binding') } }
const debug = require('./config').debug('amqp:Connection')
const {EventEmitter} = require('events')
const net = require('net')
const tls = require('tls')
const async = require('async')
const {once, defaults: applyDefaults, defer, flatten, delay, keys} = require('lodash')
const defaults = require('./defaults')
const {classes, methods} = require('./config').protocol
const {HeartbeatFrame, EndFrame} = require('./config').constants
const {serializeInt, serializeFields} = require('./serializationHelpers')
const Queue = require('./Queue')
const Exchange = require('./Exchange')
const AMQPParser = require('./AMQPParser')
const ChannelManager = require('./ChannelManager')

// todo get rid of env dependencies
if (process.env.AMQP_TEST != null) {
  defaults.connection.reconnectDelayTime = 100
}

/**
 * @typedef {Buffer & {used?: number}} ExtendedBuffer
 */

/**
 * @typedef {string | Array<string> | Array<{host: string, port: number}>} AMQPHost
 */

/**
 * @typedef {object} AMQPConnectionOptions
 * @property {any} [host="localhost"]
 * @property {string} [login="guest"]
 * @property {string} [password="guest"]
 * @property {string} [vhost="/"]
 * @property {number} [port=5672]
 * @property {boolean} [ssl=false]
 * @property {number} [sslPort=5671]
 * @property {number} [heartbeat=10000] in ms
 * @property {boolean} [reconnect=true]
 * @property {number} [reconnectDelayTime=1000] in ms
 * @property {boolean} [hostRandom=false]
 * @property {number} [connectTimeout=30000] in ms
 * @property {number} [channelMax=0] unlimited
 * @property {number} [frameMax=import('./constants').MaxFrameSize]
 * @property {boolean} [noDelay=true] disable Nagle's algorithm by default
 * @property {number} [temporaryChannelTimeout=2000] in ms
 * @property {number} [temporaryChannelTimeoutCheck=1000] in ms
 */
...
class Connection extends EventEmitter {
  ...
  constructor (args, cb) {
    ...
    this.cb = cb ? once(cb) : cb
    /** @type {'opening' | 'open' | 'closed' | 'reconnecting' | 'destroyed'} */
    this.state = 'opening'
    this.connectionOptions = applyDefaults(args, defaults.connection)
    // setup our defaults
    // this.channelCount = 0
    this.channels = {0: this}
    this.queues = {}
    // this.exchanges = {}
  ...
  // seems this method is used in tests only
  crashOOO () {
    boundMethodCheck(this, Connection)
    if (process.env.AMQP_TEST == null) {
      return true
  ...
  _connectedFirst () {
    boundMethodCheck(this, Connection)
    return debug(1, () => `Connected to ${this.connectionOptions.host}:${this.connectionOptions.port}`)
  }
  ...
  _sendMethod (channel, method, args) {
    ...
    debug(3, () => `${channel} < ${method.name}`) // #{util.inspect args}"
    b = this.sendBuffer
    b.used = 0
  ...
  _sendHeader (channel, size, args) {
    var b, bodyEnd, bodyStart, classInfo, field, headerBuffer, i, j, k, len, len1, lengthStart,
      propertyFlag, propertyFlags, ref
    boundMethodCheck(this, Connection)
    debug(3, () => `${this.id} ${channel} < header ${size}`) // #{util.inspect args}"
    b = this.sendBuffer
  ...
  _sendBody (channel, body, args, cb) {
    ...
    if (body instanceof Buffer) {
      ...
      while (offset < body.length) {
        ...
        debug(3, () => `${this.id} ${channel} < body ${offset}, ${offset + length} of ${body.length}`)
  ...
  _onContentHeader (channel, classInfo, weight, properties, size) {
    ...
    if ((channel != null ? channel._onContentHeader : void 0) != null) {
      ...
      return debug(1, () => [`unhandled -- _onContentHeader ${channel} > `, {classInfo, properties, size}])
    }
  }
  ...
  _onContent (channel, data) {
    ...
    if ((channel != null ? channel._onContent : void 0) != null) {
      ...
      return debug(1, () => `unhandled -- _onContent ${channel} > ${data.length}`)
    }
  }

  _onMethod (channel, method, args) {
    var serverVersionError
    boundMethodCheck(this, Connection)
    this._resetHeartbeatTimer()
    if (channel > 0) {
      if (this.channels[channel] == null) {
        return debug(1, () => `Received a message on untracked channel ${channel}, ${method.name} ${JSON.stringify(args)}`)
      }
      if (this.channels[channel]._onChannelMethod == null) {
        return debug(1, () => `Channel ${channel} has no _onChannelMethod`)
      }
      ...
      switch (method) {
        case methods.connectionStart:
          if (args.versionMajor !== 0 && args.versionMinor !== 9) {
            serverVersionError = new CodedError('Bad server version', 'badServerVersion')
        ...
        case methods.connectionClose:
          ...
          /** @type CodedError */
          const e = new CodedError(args.replyText, args.replyCode)
          return this.emit('close', e)
        case methods.connectionCloseOk:
          this.emit('close')
          return this.connection.destroy()
        default:
          return debug(1, () => `0 < no matched method on connection for ${method.name}`)
      }
    }
  }
}
